...
//Universal code which can be used for implementing shared memory without openFrameworks
//For example, in Unreal Engine (tested for Windows)
...
public:
    ...
    void setup(std::string memKey, int size, bool server=true);
...
template <typename T>
void ofxSharedMemory<T>::setup(std::string memoryKey, int memorySize, bool isServer) {
